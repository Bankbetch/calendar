...
module.exports = (
  ...
  res.success = (data = '', statusCode = 200) => {
    logger.http(
      `${ip} [${moment(new Date()).format('DD-MM-YYYY HH:mm:ss')}] "${
        req.method
      } ${req.originalUrl}" ${statusCode}  - "Ratelimit ${
        res.getHeaders()['x-ratelimit-remaining']
  ...
  res.error = (error) => {
    logger.http(
      `${ip} [${moment(new Date()).format('DD-MM-YYYY HH:mm:ss')}] "${
        req.method
      } ${req.originalUrl}" ${error.status}  - "Ratelimit ${
        res.getHeaders()['x-ratelimit-remaining']
      }" "${req.header('user-agent')}"`
    )
